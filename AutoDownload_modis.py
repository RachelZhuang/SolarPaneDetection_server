# -*- coding: utf-8 -*-
"""
Created on Mon Mar  7 03:14:54 2016
...
postData = {'username':'jams_hui',
            'password':'zxc2399717',
            '__ncforminfo':'sixsY-BtP8-mNajlGgFTI9JE7Xvtc7th6weygG9u0X7eND7XNeatgWteu9ozhQBl-FonScN_LH_ow3v3Hh1K0uwPZaL9jRlvNw6WYpes1lM='
...
headers2={'Content-Type':'application/x-www-form-urlencoded', 
          ...
          'User-Agent':'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/36.0.1985.125 Safari/537.36', 
          'X-Requested-With':'XMLHttpRequest'}
data2={'data':'{"tab":2,"destination":4,"cList":["4278"],"selected":4278}'}
...
soup=BeautifulSoup(searchtext,"lxml")
print (16)
all_results=soup.find_all("img")
for result in all_results:
            patt='=\"(\d*?)\"'   #正则匹配
            data=''+str(result)  #将标签字符串化
            m=re.findall(patt,data)
            num=m[0] #提出列表中的元素，即为字符串。比如：m=['12333456'],m[0]='12333456'
            print(num)
#          if m is not None:
#            num=m.group()
#            print(num)
...
#            #        print name.strip()  #文件名的前后有很多空格，name.strip()的作用就是消除字符串里的空格
            Request_URL='http://earthexplorer.usgs.gov/download/4278/'+num+'/STANDARD/EE'
            #Request_URL='http://earthexplorer.usgs.gov/download/4278/2223199028/STANDARD/EE'
            print(17)
            response=urllib.request.urlopen(Request_URL)
            ...
            #today="03.01"
            path='E:\Data\Modisdata\wuhan\\2016\\'+today+''
            if not (os.path.exists(path)): #判断是否存在目录，不存在则创建。
                 
                os.makedirs("E:\\Data\\Modisdata\\wuhan\\2016\\"+today+"")
            #path='E:\Data\Modisdata\wuhan\\2016\\'+today+''
            local=''+path+'\\'+num+'.hdf'
            #local=''+path+'\\'+num
            urllib.request.urlretrieve(Request_URL, local, callbackfunc)
#            config={'host':'90.0.0.176',#默认127.0.0.1
#                    'user':'root',
#                    'password':'123456789',
#                    'port':3306 ,#默认即为3306
#                    'database':'data_206',
...
#            sql_insert="insert into data (filename, filedate,savepath,size) values ('"+num+"', '"+start+"','"+path.replace("\\", "\\\\")+"','"+filesize+"')"
#            cursor.execute(sql_insert)
#            cursor.close()
#            cnn.close()
